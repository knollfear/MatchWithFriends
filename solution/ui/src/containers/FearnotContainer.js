...
import {Rules, aTeamRule} from "../helpers/Rules"
...
export class fearNotContainer  extends LitElement {
  ...
  fetchData() {
    return fetch('https://3bq9li6123.execute-api.us-east-1.amazonaws.com/knoll/teammembers')
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        ...
        setWithExpiry('data', this.data, 60*60*24*1000)
    ...
    if ((this.gameState.homeScore || this.gameState.awayScore) && (!this.gameState.awayLives || !this.gameState.homeLives)){
      return (html `
        <div>
          ...
          <button @click=${() => this.setupGame()}>Start New Game</button>
        </div>
      `)
    }
